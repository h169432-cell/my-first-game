(() => {
  const parts = window.__newSuspectGridParts || [];
  if (parts.length !== 4) {
    console.warn('New suspect artwork is incomplete:', parts.length);
    return;
  }

  const gridUrl = `data:image/webp;base64,${parts.join('')}`;
  document.documentElement.style.setProperty('--suspect-grid', `url("${gridUrl}")`);
  ...
  style.textContent = `
    ...
    .suspect-art,.private-suspect-art,.accuse-thumb{background-image:var(--suspect-grid);background-repeat:no-repeat;background-size:400% 200%}
  ...
  function decorateBoard() {
    ...
    state.board.forEach((card, index) => {
      ...
      art.className = `suspect-art ${cls(card.suspectNumber)}`;
  ...
  renderBoard = function renderBoardWithFineArtSuspects() {
    baseRenderBoard();
    decorateBoard();
  };
