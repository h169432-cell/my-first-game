...
3. Continue normal catalog expansion with Keyflower.
...
- Next game for catalog expansion: Keyflower.
- Current published guide count: 47 total, including Clueverge.

## Completed in this run
- Added `games/terra-mystica.html` using Capstone Games official product information and BoardGameGeek for specification corroboration.
- Added `games/hansa-teutonica-big-box.html` using Pegasus Spiele official product information and BoardGameGeek for specification corroboration.
- Added both titles to `games.html` and raised the catalog count to 47.
- Applied `strategy` to both new titles; no unsupported beginner/party/quick/two-player tags were added.
- Updated the compact TOP count link from 45 to 47 without adding featured cards.
- Used no new product images because explicit broadly reusable rights were not established.

## Files changed in this run
- `boardgame-guide/games/terra-mystica.html` (new)
- `boardgame-guide/games/hansa-teutonica-big-box.html` (new)
...
- Catalog initial count and TOP count are both 47.
- Existing category/filter keys are unchanged.
- TOP still contains only the existing six featured games.
- Terra Mystica specifications and core flow were checked against current Capstone Games and BoardGameGeek information.
- Hansa Teutonica: Big Box specifications and core flow were checked against the current Pegasus Spiele product page and BoardGameGeek.
