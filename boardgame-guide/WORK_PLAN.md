...
The published catalog now contains 78 works. `The White Castle` and `Forest Shuffle` are fully linked from `games.html` with box-visible thumbnails and visible source links. `The White Castle` uses the verified `strategy` filter; `Forest Shuffle` has no category filter because none of the current filter labels was added without direct support. `index.html` remains compact and now shows `全78作品を見る` while keeping the same six featured titles. At the next run, begin with `Earth`, first confirming that it is still absent from the repository, then verify current specifications and rules from the publisher or another reliable current source and secure a box-visible image that satisfies the image policy before publication.