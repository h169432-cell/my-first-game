...
The published catalog now contains 76 works. `Arcs` and `Sky Team` are fully linked from `games.html` with clearly box-visible official/distributor thumbnails and visible source links. Arcs uses only the verified `strategy` filter; Sky Team uses `two quick`, supported by its official two-player-only and approximately 20-minute specifications. `index.html` remains compact and now shows `全76作品を見る` while keeping the same six featured titles. At the next run, begin with `The White Castle`, which is not yet present in the repository. Reconfirm current facts from Devir's official product page; current official information supports 1–4 players, about 60 minutes, age 12+, with resource management, worker placement, and dice placement. Confirm a box-visible image that satisfies the image policy before publication, create the Japanese detail page, add its catalog card in the same run, and only then update the count from 76 to 77.
