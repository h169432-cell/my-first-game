...
Four detail pages are created and verified but are not yet linked from the catalog: `games/on-mars.html`, `games/weather-machine.html`, `games/food-chain-magnate.html`, and `games/horseless-carriage.html`. At the next run, first obtain a safe complete editable copy of the current `games.html`; current connector reads are truncated and the available write operation replaces the whole file, so do not risk a partial reconstruction. Then publish all four cards with box-visible thumbnails, visible source links, and only the `strategy` tag. On Mars and Weather Machine have re-verified official Eagle-Gryphon Games product sources. Food Chain Magnate's current official Splotter page is a 13th-print-run pre-order and remains the preferred source. Horseless Carriage 2nd Edition remains listed in Splotter's current catalog. After all four cards are linked, update the catalog from 69 to 73 and the TOP total-count button accordingly, then continue normal expansion with another not-yet-published popular title.
