...
Four detail pages are now created and verified but are not yet linked from the catalog: `games/on-mars.html`, `games/weather-machine.html`, `games/food-chain-magnate.html`, and `games/horseless-carriage.html`. At the next run, safely retrieve/reconstruct the complete current `games.html` and publish all four first. Use box-visible catalog thumbnails with visible source links and only the `strategy` tag. After all four cards are linked, update the catalog from 69 to 73 and the TOP total-count button accordingly, then continue normal expansion with another not-yet-published popular title.
